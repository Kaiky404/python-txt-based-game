...
def principal():
    ...
    print(F"{C.BLUE}Seja bem-vindo à The Village")
    print(visuals.introdução)
    print(F"{C.BLUE}Este é um jogo de aventura em texto feito inteiramente em Python.\n")
    ...
    while True:
        evento.cabecalho('narrador')
        print("Você está em seu quarto")
        print(visuals.quarto)
        escolhaPrincipal = helpers.pergunta(
            'escolha',
            [F'Você pode olhar a {C.YELLOW}janela{C.NORMAL}, olhar o {C.YELLOW}quarto{C.NORMAL} mais a fundo ou descer a {C.YELLOW}escada{C.NORMAL}'],
        ...
        if escolhaPrincipal is None:
            ...
            print(visuals.escadas_descendo)
            result = escada()
            if result == "voltar":
                continue
